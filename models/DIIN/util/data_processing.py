...
FIXED_PARAMETERS, config = load_parameters()
...
em_cache_stem, em_cache_synsets, em_cache_lemma_names = {}, {}, {}

def get_stem(token):
    if not token in em_cache_stem:
        em_cache_stem[token] = stemmer.stem(token)
    return em_cache_stem[token]

def get_synsets(token):
    if not token in em_cache_synsets:
        em_cache_synsets[token] = wn.synsets(token)
    return em_cache_synsets[token]

def get_lemma_names(synset):
    if not synset in em_cache_lemma_names:
        em_cache_lemma_names[synset] = synset.lemma_names()
    return em_cache_lemma_names[synset]
...
def is_exact_match(token1, token2):
    ...
    #             return True
    for synset in get_synsets(token2):
        for lemma in get_lemma_names(synset):
    ...
    if token1 == "n't" and token2 == "not":
        ...
        return True
    elif token1_stem == get_stem(token2): #stemmer.stem(token2):
        return True
    return False
...
def get_em_and_ner(example):
    ...
    example["sentence1_NER_feature"] = get_ner(example["sent_a_pos"])
    example["sentence2_NER_feature"] = get_ner(example["sent_b_pos"])
...
def sentences_to_padded_index_sequences(datasets):
    ...
    for i, dataset in enumerate(datasets):
        ...
        for example in tqdm(dataset):
            ...
            # all_tokenize.append((s1_tokenize, s2_tokenize))
            s1_tokenize = example["sent_a"]
            s2_tokenize = example["sent_b"]

            # for i in range(len(s1_tokenize)):
            #     assert(s1_tokenize[i] == example["sent_a"][i])

            # assert(len(s1_tokenize) == len(example["sent_a"]))
            # assert(len(s2_tokenize) == len(example["sent_b"]))

            example["sentence1_part_of_speech_tagging"] = " ".join([w[1] for w in example["sent_a_pos"]]) 
            example["sentence2_part_of_speech_tagging"] = " ".join([w[1] for w in example["sent_b_pos"]])
        ...
        num_cpus = 32
        pbar.set_total(len(dataset) / num_cpus)

        with Pool(processes=num_cpus) as pool:
            dataset = pool.map(get_em_and_ner, dataset)
            datasets_new.append(dataset)
            # exact_match = pool.map(get_exact_match_features, all_tokenize)

        # for i, example in enumerate(dataset):
        #     example["sentence1_token_exact_match_with_s2"] = exact_match[i][0]
        #     example["sentence2_token_exact_match_with_s1"] = exact_match[i][1]
                ...
                for i in range(FIXED_PARAMETERS["seq_length"]):
                    if i >= len(token_sequence):
                        ...
                        if config.embedding_replacing_rare_word_with_UNK:
                            ...
                            if not token_sequence[i] in word_indices:
                                index = 0
                            else:
    ...
    return datasets, indices_to_words, word_indices, char_indices, indices_to_char
