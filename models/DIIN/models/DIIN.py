...
class MyModel(object):
    def __init__(self, config, seq_length, emb_dim, hidden_dim, emb_train,  embeddings = None, pred_size = 3, context_seq_len = None, query_seq_len = None):
        ...
        self.premise_ner = tf.placeholder(tf.int32, [None, self.sequence_length,7], name='premise_ner')
        self.hypothesis_ner = tf.placeholder(tf.int32, [None, self.sequence_length,7], name='hypothesis_ner')
        ...
        premise_in = tf.concat([premise_in, tf.cast(self.premise_ner, tf.float32)], axis=2)
        hypothesis_in = tf.concat([hypothesis_in, tf.cast(self.hypothesis_ner, tf.float32)], axis=2)
        ...
        self.error = 1 - self.prob[:, 0]                      

        tf.summary.histogram('logit_histogram', self.logits)

        # Define the cost function
        self.total_cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y, logits=self.logits))
        self.acc = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.logits, axis=1),tf.cast(self.y,tf.int64)), tf.float32))
