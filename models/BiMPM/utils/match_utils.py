...
eps = 1e-6
def cosine_distance_here(y1,y2):
    # y1 [....,a, 1, d]
    # y2 [....,1, b, d]
...
def cal_relevancy_matrix(in_question_repres, in_passage_repres):
    ...
    relevancy_matrix = cosine_distance_here(in_question_repres_tmp,in_passage_repres_tmp) # [batch_size, passage_len, question_len]
...
def cal_maxpooling_matching(passage_rep, question_rep, decompose_params):
    ...
    def singel_instance(x):
        ...
        return cosine_distance_here(p, q) # [passage_len, question_len, decompose]
...
def multi_perspective_match(feature_dim, repres1, repres2,  dropout_rate=None, 
    ...
    with tf.variable_scope(scope_name, reuse=reuse):
        ...
        if options.with_cosine:
            cosine_value = cosine_distance(repres1, repres2, cosine_norm=False)
        ...
        if options.with_mp_cosine:
            ...
            mp_cosine_matching = cosine_distance(tf.multiply(repres1_flat, mp_cosine_params),
...
def match_passage_with_question(passage_reps, question_reps, passage_mask, question_mask, passage_lengths, question_lengths,
                                ...
                                dropout_rate=None, forward=True):
    ...
    with tf.variable_scope(scope or "match_passage_with_question"):
        ...
        if with_full_match:
            if forward:
                question_full_rep = collect_final_step_of_lstm(question_reps, question_lengths - 1)
            else:
            ...
            (attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
                                passage_reps, question_full_rep, dropout_rate=dropout_rate,
        ...
        if with_attentive_match:
            atten_scores = calcuate_attention(passage_reps, question_reps, context_lstm_dim, context_lstm_dim,
                    scope_name="attention", att_type=options.att_type, att_dim=options.att_dim,
                    remove_diagnoal=False, mask1=passage_mask, mask2=question_mask, dropout_rate=dropout_rate)
            att_question_contexts = tf.matmul(atten_scores, question_reps)
            (attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
                    passage_reps, att_question_contexts, dropout_rate=dropout_rate,
        ...
        if with_max_attentive_match:
            ...
            (max_attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
                    passage_reps, max_att, dropout_rate=dropout_rate,
...
def bilateral_match_func(in_question_repres, in_passage_repres,
                        question_lengths, passage_lengths, question_mask, passage_mask, input_dim, dropout_rate=None):
    ...
    (match_reps, match_dim) = match_passage_with_question(in_passage_repres, in_question_repres, passage_mask, question_mask, passage_lengths,
                                ...
                                dropout_rate=dropout_rate, forward=True)
    ...
    (match_reps, match_dim) = match_passage_with_question(in_question_repres, in_passage_repres, question_mask, passage_mask, question_lengths,
                                ...
                                with_attentive_match=options.with_attentive_match,
                                with_max_attentive_match=options.with_max_attentive_match,
                                dropout_rate=dropout_rate, forward=False)
            ...
            with tf.variable_scope('layer-{}'.format(i)):
                ...
                (question_context_representation_fw, question_context_representation_bw,
                 in_question_repres) = my_lstm_layer(
                        ...
                        reuse=False, dropout_rate=dropout_rate)
                (passage_context_representation_fw, passage_context_representation_bw, 
                 in_passage_repres) = my_lstm_layer(
                        ...
                        reuse=True, dropout_rate=dropout_rate)
    ...
    with tf.variable_scope("left_matching_highway"):
        question_aware_representatins = multi_highway_layer(question_aware_representatins, question_aware_dim, 1)
    with tf.variable_scope("right_matching_highway"):
        passage_aware_representatins = multi_highway_layer(passage_aware_representatins, passage_aware_dim, 1)
    ...
    with tf.variable_scope('aggregation_layer'):
        for i in range(options.aggregation_layer_num): # support multiple aggregation layer
            ...
            (fw_rep, bw_rep, cur_aggregation_representation) = my_lstm_layer(
                        ...
                        reuse=False, dropout_rate=dropout_rate)
            fw_rep = collect_final_step_of_lstm(fw_rep, passage_lengths - 1)
            bw_rep = bw_rep[:, 0, :]
            ...
            (fw_rep, bw_rep, cur_aggregation_representation) = my_lstm_layer(
                        ...
                        reuse=False, dropout_rate=dropout_rate)
            fw_rep = collect_final_step_of_lstm(fw_rep, question_lengths - 1)
            bw_rep = bw_rep[:, 0, :]
            aggregation_representation.append(fw_rep)
            aggregation_representation.append(bw_rep)
            aggregation_dim += 2* options.aggregation_lstm_dim
    ...
    with tf.variable_scope("aggregation_highway"):
        ...
        aggregation_representation = multi_highway_layer(aggregation_representation, aggregation_dim, 1)
